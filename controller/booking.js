...
router.get('/tables/:whento', function(req, res){
    ...
    // Check if the target date exceeds the number of days in the current month
    if (targetDate.getMonth() !== today.getMonth()) {
        // Move to the next month
        targetDate.setMonth(today.getMonth() + 1);
        // Set the day to the appropriate value
        targetDate.setDate(shiftBy - (currentDay - 1));
    }
    ...
    const justNow = today.toLocaleDateString('en-US', options);
    ...
    let query = db.query(sql, [dayOfWeek], (err, results) => {
      if(err) {
        ...
        // Usage example
        //const justNow = 'Monday'; // Assuming justNow represents the current day of the week
