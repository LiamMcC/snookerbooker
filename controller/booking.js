...
router.get('/tables/:whento/:club', function(req, res){
    ...
    let sql = ` SELECT * FROM webcontent WHERE location = "Bookings";
                    SELECT * FROM gametables WHERE club = ? ;
                    SELECT * FROM slottimes WHERE daychoice = ? `;
  
    let query = db.query(sql, [req.params.club, dayOfWeek], (err, results) => {
      if(err) {
        throw(err)
...
const actualDay = new Date().toLocaleDateString('en-US', { weekday: 'long' });
     ...
     //   console.log("Actual Day Now Liam " + actualDay )
        ...
        res.render('layout', { tableData, siteData, dayOfWeek, formattedDate, nextDays, slotData, user: req.user });
      }
    });
});
...
function insertBooking(username, email, phone, dayOfWeek, date, tableNo, club) {
    return new Promise((resolve, reject) => {
      const sql = `INSERT INTO bookings (username, email, phone, dayOfWeek, whatdate, tableNo, club) VALUES (?, ?, ?, ?, ?, ?, ?)`;
      db.query(sql, [username, email, phone, dayOfWeek, date, tableNo, club], (err, result) => {
        if (err) {
          reject(err);
        } else {
...
router.get('/cancel/:identifier',  function(req, res){
  const currentRoute = req.url;

  let sql = 'UPDATE bookedslots set slotTime = "00:00" WHERE bookref = ?';
  let query = db.query(sql, [req.params.identifier],(err,result) => {
    if(err) {
      res.redirect('/error'); // Redirect to error page
    } else { 
      res.render('cancelled', {currentRoute});
    }
  });
  ...
  function insertBookedSlots(bookingId, slotTimes, randomString, club) {
      return new Promise((resolve, reject) => {
      const sql = `INSERT INTO bookedslots (bookingId, slotTime, bookref, club) VALUES ?`;
      const values = slotTimes.map(slotTime => [bookingId, slotTime,randomString, club]);
    ...
    try {
      ...
      function generateRandomString(length) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            refstring += characters.charAt(randomIndex);
        }
  ...
  //console.log(randomString);
      ...
      const bookingId = await insertBooking(username, email, phone, dayOfWeek, formattedDate, tableNumber, club);
  
      // Insert booked slots for the booking ID into the bookedSlots table
      const rowsInserted = await insertBookedSlots(bookingId, timeSlots, randomString, club);
  
      //console.log(`${rowsInserted} slot(s) inserted for the booking`);
      Email.bookingConfirmed(email, timeSlots, bookingId,  dayOfWeek, formattedDate, tableNumber, randomString, club)
      // Send a response
